...
def update_score(l_score, r_score, player, score_board):
    ...
    score_board.clear()
    score_board.write('Left player:{}---Right player:{}'.format((l_score, r_score)), align='center', font=('Arial', 24,'normal'))

    return l_score, r_score, score_board

def setup_game():
    screen = turtle.Screen()
    screen.title('Pong Acade Game')
...
def pong_game():
    ...
    r_paddle = game_components[2]
    ball = game_components[3]
    ...
    screen.listen()
    screen.onkeypress(l_paddle, 'e')
    screen.onkeypress(l_paddle, 'x')
    screen.onkeypress(r_paddle, 'Up')
